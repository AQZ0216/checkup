...
// BNCLedgerChecker implements a Checker for Binance chain ledger endpoints.
type BNCLedgerChecker struct {
...
func (c BNCLedgerChecker) Check() (Result, error) {
	if c.Attempts < 1 {
		c.Attempts = 1
	}
	if c.Client == nil {
	...
	t := time.Now().Unix()
	url := fmt.Sprintf("%s%s?ac=%s&t=%d", c.URL, "/v1/bnb/block/latestblockinfo", c.MyAPICode, t)
...
func (c BNCLedgerChecker) doChecks(req *http.Request) Attempts {
...
func (c BNCLedgerChecker) conclude(result Result) Result {
...
func (c BNCLedgerChecker) checkDown(resp *http.Response) error {
	...
	lastBNBCheckBlockNum := checkresult.BlockNumber
...
func (c BNCLedgerChecker) getHeightDataseed1() (int64, error) {
...
func (c BNCLedgerChecker) getHeightDEXAPI() (int64, error) {
