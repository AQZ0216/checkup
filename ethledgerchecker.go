...
import (
	...
	eapi "github.com/nanmu42/etherscan-api"
	"github.com/sourcegraph/checkup/utils"
)

// ETHLedgerChecker implements a Checker for ETH ledger endpoints.
type ETHLedgerChecker struct {
...
func (c ETHLedgerChecker) Check() (Result, error) {
	if c.Attempts < 1 {
		c.Attempts = 1
	}
	if c.Client == nil {
	...
	t := time.Now().Unix()
	url := fmt.Sprintf("%s%s?ac=%s&t=%d", c.URL, "/v1/eth/block/latestblockinfo", c.MyAPICode, t)
...
func (c ETHLedgerChecker) doChecks(req *http.Request) Attempts {
...
func (c ETHLedgerChecker) conclude(result Result) Result {
...
type blockNumberResultInt64 struct {
...
func (c ETHLedgerChecker) checkDown(resp *http.Response) error {
	// Check status code
	...
	var etherScanBlockHeight int64
	url := "etherscanapi"
	responsebodyStr := utils.GlobalCacheGetString(url)
	if responsebodyStr == "" {
		// get etherscan blockheight
		client := eapi.New(eapi.Mainnet, c.APIKey)

		etherScanBlockHeight, err = client.BlockNumber()
		if err != nil {
			return err
		}

		var str = strconv.FormatInt(etherScanBlockHeight, 10)
		utils.GlobalCacheSetString(url, str)
	} else {
		log.Printf("ETH(%s) use cache key: %s", c.Name, url)
		etherScanBlockHeight, err = strconv.ParseInt(responsebodyStr, 0, 64)
		if err != nil {
			return err
		}
	}

	log.Printf("%s, ref:%s BlockHeight:%d      check:%s last block BlockHeight:%d\n", c.Name, url, etherScanBlockHeight, c.URL, currentBlockNumber)

	blockDiff := etherScanBlockHeight - currentBlockNumber
	if blockDiff > c.BlockHeightBehind {
		return fmt.Errorf("blockheight(%d) was behind etherscan(%d) %d blocks, threshold(%d)", currentBlockNumber, etherScanBlockHeight, blockDiff, c.BlockHeightBehind)
	}
