...
// ETHChecker implements a Checker for ETH endpoints.
...
func (c ETHChecker) Check() (Result, error) {
	if c.Attempts < 1 {
		c.Attempts = 1
	}
	if c.Client == nil {
	...
	req, err := http.NewRequest("POST", c.URL, bytes.NewBuffer([]byte(c.Param)))
...
func (c ETHChecker) doChecks(req *http.Request) Attempts {
...
func (c ETHChecker) conclude(result Result) Result {
...
func (c ETHChecker) checkDown(resp *http.Response) error {
	// Check status code
	...
	// parse blockheight
	var result = new(blockNumberResult)
	err = json.Unmarshal(bodyBytes, result)
	if err != nil {
		return err
	}

	currentBlockNumber, err := strconv.ParseInt(result.BlockNumber, 0, 64)
	if err != nil {
		return err
	}
