...
// EOSChecker implements a Checker for EOS endpoints.
type EOSChecker struct {
...
func (c EOSChecker) Check() (Result, error) {
	if c.Attempts < 1 {
		c.Attempts = 1
	}
	if c.Client == nil {
...
func (c EOSChecker) doChecks(req *http.Request) Attempts {
...
func (c EOSChecker) conclude(result Result) Result {
...
func (c EOSChecker) checkDown(resp *http.Response) error {
	...
	log.Printf("%s, ref:%s BlockHeight:%d      check:%s last block BlockHeight:%d\n", c.Name, url, blockInfo.LastIrreversibleBlockNum, c.URL, result.LastIrreversibleBlockNum)

	blockDiff := blockInfo.LastIrreversibleBlockNum - result.LastIrreversibleBlockNum
	if (blockInfo.LastIrreversibleBlockNum > result.LastIrreversibleBlockNum) && (blockDiff > c.BlockHeightBehind) {
		return fmt.Errorf("%s blockheight(%d) was behind eos node tool(%d) %d blocks, threshold(%d)", c.Name, result.LastIrreversibleBlockNum, blockInfo.LastIrreversibleBlockNum, blockDiff, c.BlockHeightBehind)
	}
